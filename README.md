...
- **92** `SynapticConfig` hyperparameters, every one read by runtime code (the 6 dead fields were pruned in `8j9.5`; the bistable-latch mechanism `sax.2` added 12 live, default-off knobs; the differentiable-recurrence wiring `hwxb.4.6` added 3 live, default-off knobs — `differentiable_recurrence`, `recurrence_block_size`, `recurrence_chunk_len`; the cusp retention certificate `0642.2.2.3` added 2 — `cusp_latch`, `cusp_eps_max`; the metriplectic integrator toggle `0642.1.2.4` added 1 — `metriplectic_integrator`). The count is machine-verified by `scripts/param_census.py`; the "48-parameter genome" figure was an early planning estimate, not a code count.
...
### Core Implementation
*   **`bio_inspired_nanochat/synaptic.py`** ⚡ **The Physics Engine**: 92-parameter `SynapticConfig` + core dynamics
