...
- **82** `SynapticConfig` hyperparameters, every one read by runtime code (the 6 dead fields were pruned in `8j9.5`; the bistable-latch mechanism `sax.2` added 12 live, default-off knobs). The "48-parameter genome" figure was an early planning estimate, not a code count.
...
### Core Implementation
*   **`bio_inspired_nanochat/synaptic.py`** ⚡ **The Physics Engine**: 82-parameter `SynapticConfig` + core dynamics
